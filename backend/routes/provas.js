...
router.get('/', async (req, res) => {
    try {
        const provas = await db.all(
            'SELECT * FROM provas WHERE user_id = ? ORDER BY data ASC',
            [req.user.userId]
        );
...
// Listar próximas provas (próximos 30 dias)
...
router.post('/', async (req, res) => {
    try {
        ...
        const result = await db.run(
            'INSERT INTO provas (titulo, data, descricao, user_id) VALUES (?, ?, ?, ?)',
            [titulo, data, descricao || '', req.user.userId]
        );

        const novaProva = await db.get(
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const prova = await db.get(
            'SELECT id FROM provas WHERE id = ? AND user_id = ?',
            [id, req.user.userId]
        );
        ...
        await db.run(
            'UPDATE provas SET titulo = ?, data = ?, descricao = ? WHERE id = ?',
            [titulo, data, descricao || '', id]
        );

        const provaAtualizada = await db.get(
            'SELECT * FROM provas WHERE id = ?',
            [id]
        );
